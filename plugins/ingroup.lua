...
local function check_member_realmrem(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, 'گروه مادری حذف شد')
    end
  end
end
local function check_member_modrem(cb_extra, success, result)
  ...
  for k,v in pairs(result.members) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      return send_large_msg(receiver, 'ربات در این گروه غیرفعال شد')
    end
  end
end
--End Check Member
...
local function set_descriptionmod(msg, data, target, about)
  ...
  save_data(_config.moderation.data, data)
  return 'توضیحات گرو تغییر یافت به:\n'..about
end
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    return 'چت ممنوع فعال شد اگر کسی حرف بزند اخراج میشود'
  else
...
local function lock_group_ads(msg, data, target)
  ...
  if group_ads_lock == 'yes' then
    return 'انتی تبلیغات فعال شد لینک=اخراج'
  else
...
local function unlock_group_ads(msg, data, target)
  ...
  if group_ads_lock == 'no' then
    return 'انتی تبلیغات غیرفعال شد'
  else
...
local function lock_group_english(msg, data, target)
  ...
  if group_english_lock == 'yes' then
    return 'استفاده از لغات انگلیسی ممنوع شد'
  else
...
local function unlock_group_english(msg, data, target)
  ...
  if group_english_lock == 'no' then
    return 'قفل کلمات انگلیسی غیرفعال شد'
  else
...
local function lock_group_tag(msg, data, target)
  ...
  if group_tag_lock == 'yes' then
    return 'امنیت مخصوص نود32 فعال شد'
  else
...
local function lock_group_join(msg, data, target)
  ...
  if group_join_lock == 'yes' then
    return 'ورود اعضا توسط لینک گروه ممنوع شد'
  else
...
local function unlock_group_join(msg, data, target)
  ...
  if group_join_lock == 'no' then
    return 'ورود اعضا توسط لینک فعال شد'
  else
...
local function lock_group_namemod(msg, data, target)
  ...
  if group_name_lock == 'yes' then
    return 'اسم گروه قفل شد و بغیر از مدیر ارشد کسی قادر به تغییر ان نمیباشد'
  else
...
local function unlock_group_namemod(msg, data, target)
  ...
  if group_name_lock == 'no' then
    return 'قفل اسم گروه غیرفعال شد'
  else
...
local function lock_group_floodmod(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    return 'حساسیت اسپم در گروه فعال شد'
  else
...
local function unlock_group_floodmod(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    return 'حساسیت به اسپم در گروه غیرفعال شد'
  else
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return 'ورود اعضا جدید به گروه ممنوع شد'
  else
...
local function unlock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'no' then
    return 'ورود اعضا جدید به گروه ازاد شد'
  else
...
local function modadd(msg)
  ...
  if is_group(msg) then
    return 'این گروه از قبل به رسمیت شناخته شدخ بود'
  end
    receiver = get_receiver(msg)
...
function modrem(msg)
  ...
  if not is_group(msg) then
    return 'این گروه اضافه نشده است'
  end
    receiver = get_receiver(msg)
...
local function get_rules(msg, data)
  local data_cat = 'rules'
  if not data[tostring(msg.to.id)][data_cat] then
    return 'هیچ قوانینی در این گروه تعریف نشده'
  end
...
local function set_group_photo(msg, success, result)
  ...
  if success then
    ...
    save_data(_config.moderation.data, data)
    send_large_msg(receiver, 'عکس قفل و ذخیره شد', ok_cb, false)
  else
...
local function promote(receiver, member_username, member_id)
  ...
  if not data[group] then
    return send_large_msg(receiver, 'این گروه اضافه نشده است')
  end
  if data[group]['moderators'][tostring(member_id)] then
    return send_large_msg(receiver, member_username..' این شخص از قبل ادمین بود')
  end
  data[group]['moderators'][tostring(member_id)] = member_username
  save_data(_config.moderation.data, data)
  return send_large_msg(receiver, member_username..' کاربر مورد نظر ادمین شد')
end
...
local function demote(receiver, member_username, member_id)
  ...
  if not data[group]['moderators'][tostring(member_id)] then
    return send_large_msg(receiver, member_username..' این کاربر از قبل برکنار شده بود')
  end
  data[group]['moderators'][tostring(member_id)] = nil
  save_data(_config.moderation.data, data)
  return send_large_msg(receiver, member_username..' این شخص از ادمینی بر کنار شد')
end
...
local function setowner_by_reply(extra, success, result)
  ...
  data[tostring(msg.to.id)]['set_owner'] = tostring(msg.from.id)
      ...
      local text = msg.from.print_name:gsub("_", " ").." مدیر ارشد فعلی گروه"
...
local function modlist(msg)
  ...
  if next(data[tostring(msg.to.id)]['moderators']) == nil then --fix way
    return 'هیچ ادمینی در این گروه اضافه نشده'
  end
  local i = 1
...
local function run(msg, matches)
  ...
  if msg.to.id and data[tostring(msg.to.id)] then
    ...
    if matches[1] == 'setname' and is_momod(msg) then
      ...
      savelog(msg.to.id, "اسم گروه { "..msg.to.print_name.." }  تغیر یافت به[ "..new_name.." ] by "..name_log.." ["..msg.from.id.."]")
    end
    if matches[1] == 'setphoto' and is_momod(msg) then
      data[tostring(msg.to.id)]['settings']['set_photo'] = 'waiting'
      save_data(_config.moderation.data, data)
      return 'عکس پروفایل گروه را بفرستید'
    end
    ...
    if matches[1] == 'promote' and matches[2] then
      ...
      if not is_owner(msg) then
        return "فقط مدیر ارشد میتواند کسی را ادمین کند"
      end
    ...
    if matches[1] == 'demote' and matches[2] then
      ...
      if not is_owner(msg) then
        return "فقط مدیر ارشد میتواند کسی را از ادمینی برکنار کند"
      end
      if string.gsub(matches[2], "@", "") == msg.from.username and not is_owner(msg) then
        return "خودتان را نمیتوانید برکنار کنید"
      end
	local member = matches[2]
    ...
    if matches[1] == 'set' then
      if matches[2] == 'rules' then
        ...
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] قوانین تغییر یافت به ["..matches[3].."]")
      ...
      if matches[2] == 'about' then
        ...
        savelog(msg.to.id, name_log.." ["..msg.from.id.."] توضیحات تغییر یافت به ["..matches[3].."]")
  ...
  --[[if matches[1] == 'public' then
    ...
    if matches[1] == 'setowner' and matches[2] then
      if not is_owner(msg) then
        return "مجاز برای مدیر ارشد"
      end
      data[tostring(msg.to.id)]['set_owner'] = matches[2]
      save_data(_config.moderation.data, data)
      savelog(msg.to.id, name_log.." ["..msg.from.id.."] set ["..matches[2].."] as owner")
      local text = matches[2].." کاربر مورد نظر مدیر ارشد  شد"
      return text
    end
    ...
    if matches[1] == 'owner' then
      ...
      return "شناسه مدیر ارشد گروه ["..group_owner..']'
    end
    if matches[1] == 'setgpowner' then
      ...
      save_data(_config.moderation.data, data)
      local text = matches[3].." کاربر مورد نظر مدیر ارشد  شد"
    ...
    if matches[1] == 'setflood' then 
      ...
      if tonumber(matches[2]) < 3 or tonumber(matches[2]) > 40 then
        return "اعداد انتخوابی باید بین 3 تا 40 باشد"
      end
      local flood_max = matches[2]
      data[tostring(msg.to.id)]['settings']['flood_msg_max'] = flood_max
      save_data(_config.moderation.data, data)
      savelog(msg.to.id, name_log.." ["..msg.from.id.."] set flood to ["..matches[2].."]")
      return 'حساسیت به اسپم تنظیم شد روی '..matches[2]
    end
    ...
    if matches[1] == 'kickinactive' then
      --send_large_msg('chat#id'..msg.to.id, 'I\'m in matches[1]')
	    if not is_momod(msg) then
	      return 'مدیر و ادمین ها میتوانند اعضا غیرفعال را اخراج کنند'
	    end
	    local num = 1
	    if matches[2] then
	        num = matches[2]
